...
#Paths of the images
HealthyImage ="/Users/aman/Desktop/Sixth Sense/PROGRAMS/Capstone/testimage/Healthy_leaf.png"
InfectedImage ="/Users/aman/Desktop/Sixth Sense/PROGRAMS/Capstone/testimage/Infected_leaf.png"

#ImageFile=HealthyImage
ImageFile=InfectedImage

"""
Procedure:
1) Take the image and then normalise it according to the lighting conditions.
2) Apply Gaussian Blur to the image & pymeanShift to downsample(lower the quality of the image) makes it faster to process.
3) Apply Canny Edge Detection to the image.
We look at the 3 channels of the image, and if the layers have different HLS(Hue, lighting , saturation) values then we normalise it.
4) Apply inverse binary masking using roi color images (we convert the darkest spots to lightest) and then the color of the infected images remains as spots.
4) Find the contours of the image.
5) Find the area of the contours.
6) Find the area of the leaf (our region of interest).
7) Find the ratio of the contour area  to the leaf area.
8) Now the area of contours give us the area of the infected leaf, gives 0 if its a healthy leaf.
...
# many images , then it takes time to open hence we divide the size by 5
...
# if per_white > 10:
# 	img[i][j] = [200,200,200] #brightens the image
...
img = cv2.pyrMeanShiftFiltering(blur1, 20, 30, newimg, 0, criteria)     # this down samples the image and hence
...
# axis 2 means that all the layers have same hue sat values
...
# disease sopts are clearly visible surrounded by dark bg
...
print("\npress y on image to run the classifier:")
n = cv2.waitKey(0) & 0xFF # all images will br still until we press something
...
while True:	
        ...
        try:
            ...
            my_df.to_csv("Datasetunlabelledlog.csv", mode='a', index=False, header=False)			
