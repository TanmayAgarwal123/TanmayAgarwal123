...
def transparent_layout(img,buttonList):
    imgNew = np.zeros_like(img,np,np.uint8)
    for button in buttonList:
        x,y=button.pos
...
while True:
    ...
    if imList:
        for button in buttonList:
            ...
            if x<imList[0][0]<x+w and y<imList[0][1]<y+h:
                ...
                if l<25:
                    ...
                    cv2.putText(img,button.pos,(x+w,y+h),(0,255,0),cv2.FILLED)
                    cv2.putText(img,button.text,(x+20,y+65),cv2.FONT_HERSHEY_PLAIN,4,(0,0,0),4)
                    final_text +=button.text
                    sleep(0.20)
cv2.rectangle(img,(25,350),(700,450),(255,255,255),cv2.FILLED)
cv2.putText(img,final_text,(60,455),cv2.FONT_HERSHEY_PLAIN,4,(0,0,0),4)
