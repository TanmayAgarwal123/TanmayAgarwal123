...
connector.execute(
"CREATE TABLE IF NOT EXISTS College_MANAGEMENT (STUDENT_ID INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, NAME TEXT, EMAIL TEXT, PHONE_NO TEXT, GENDER TEXT, DOB TEXT, STREAM TEXT)"
)
...
def add_record():
   ...
   if not name or not email or not contact or not gender or not DOB or not stream:
       ...
       try:
           connector.execute(
           'INSERT INTO College_MANAGEMENT (NAME, EMAIL, PHONE_NO, GENDER, DOB, STREAM) VALUES (?,?,?,?,?,?)', (name, email, contact, gender, DOB, stream)
           )
           connector.commit()
...
main = Tk()
main.title('DataFlair College Management System')
